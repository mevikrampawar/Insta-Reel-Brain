...
export function IngestionForm({ addReel, updateReel, onDone }: Props) {
    ...
    return (
      <div className="max-w-2xl mx-auto p-8">
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-6 text-center space-y-3">
          ...
          <h3 className="font-medium text-amber-300">API Key Not Configured</h3>
          <p className="text-sm text-zinc-400">
            Get a free Groq API key from{' '}
            <a href="https://console.groq.com" target="_blank" rel="noopener" className="text-indigo-400 underline">console.groq.com</a>
          </p>
          <p className="text-xs text-zinc-500">
            Then add it to your .env file: VITE_GROQ_API_KEY=gsk_...
          </p>
        </div>
      </div>
    )
  ...
  const handleUrlSubmit = async () => {
    ...
    try {
      const meta = await scrapeInstagramUrl(url)
      setTitle(meta.title)
      ...
      // If scraping fails, still let user proceed manually
      setStep('review')
      setError('Could not auto-fetch metadata. Please fill in details below.')
    }
  }

  // Step 2: Review + paste transcript → Process
      ...
      {isUrlStep && (
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={step === 'fetching' || !url.trim()}
          ...
          >
            {step === 'fetching' ? (
              ...
              <><Sparkles size={16} /> Auto-extract & Continue</>
            )}
          </button>
          <p className="text-xs text-zinc-600 text-center">
            Or skip — paste the URL and transcript manually below
          </p>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          {!thumbnailUrl && (
            <div className="grid grid-cols-2 gap-3">
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Title (auto-detected)"
          ...
          <input value={caption} onChange={e => setCaption(e.target.value)} placeholder="Caption (auto-detected)"
            className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500" />
          <input value={hashtags} onChange={e => setHashtags(e.target.value)} placeholder="Hashtags (auto-detected, comma-separated)"
            className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500" />
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Paste the Reel's transcript, captions, or describe the content in detail. The more you provide, the better the AI analysis."
              rows={8}
            ...
            <p className="text-xs text-zinc-600 mt-1">
              AI will analyze this to generate summary, takeaways, tags, and searchable embeddings.
            </p>
          </div>
